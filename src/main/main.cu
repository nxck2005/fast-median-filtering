...
void run_benchmark(const std::string& filename, int max_ksize) {
    ...
    cv::Mat dst_ht; // Declare dst_ht here to access it after the loop
    ...
    cv::imwrite("FILT_old.jpg", src);
    cv::imwrite("FILT_new.jpg", dst_ht);
    std::cout << "\nSaved original image to FILT_old.jpg" << std::endl;
    std::cout << "Saved filtered image to FILT_new.jpg" << std::endl;
