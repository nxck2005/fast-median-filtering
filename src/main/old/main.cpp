...
// Templated function to run the benchmark for a specific pixel type
template <typename T>
void run_benchmark(const std::string& filename, int max_ksize) {
    ...
    // Convert image to the target type and ensure it's single channel for simplicity
    cv::Mat src;
    ...
    for (int k = 7; k <= max_ksize; k += 2) {
        ...
        // --- Benchmark OpenCV ---
        cudaEvent_t start_ocv, stop_ocv;
        cudaEventCreate(&start_ocv);
        cudaEventCreate(&stop_ocv);
        
        cv::cuda::GpuMat d_src, d_dst_ocv;
        d_src.upload(src);
        
        // Warm-up run
        cv::medianBlur(src, dst_ocv, k);

        cudaEventRecord(start_ocv);
        cv::medianBlur(src, dst_ocv, k); // Using CPU version as cv::cuda::medianFilter doesn't exist
        cudaEventRecord(stop_ocv);
        cudaEventSynchronize(stop_ocv);
        float time_ocv;
        cudaEventElapsedTime(&time_ocv, start_ocv, stop_ocv);
        opencv_times.push_back(time_ocv);
        cudaEventDestroy(start_ocv);
        cudaEventDestroy(stop_ocv);
        
        // --- Benchmark Hierarchical Tiling ---
        ...
        float time_ht;
        cudaEventElapsedTime(&time_ht, start_ht, stop_ht);
        ht_times.push_back(time_ht);
        cudaEventDestroy(start_ht);
        cudaEventDestroy(stop_ht);

        // Print results for this kernel size
        std::cout << std::left << std::setw(15) << (std::to_string(k) + "x" + std::to_string(k))
                  << std::setw(20) << std::fixed << std::setprecision(2) << time_ocv
                  << std::setw(30) << time_ht
                  << std::setw(15) << (time_ocv / time_ht) << "x" << std::endl;
    }
...
int main(int argc, char* argv) {
    ...
    if (!get_cmd_option(argv, argv + argc, "--filename", filename) ||
       ...
       !get_cmd_option(argv, argv + argc, "--max_ksize", max_ksize_str)) {
        std::cerr << "Usage: " << argv << " --filename <path> --type <uint8|uint16|float> --max_ksize <size>" << std::endl;
        return -1;
    }
